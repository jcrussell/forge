...
describe('Bug #172: Per-window float toggle', () => {
  ...
  describe('FloatToggle (per-window)', () => {
    it('should only float the specific window when using FloatToggle', () => {
      ...
      const nodeWindow1 = windowManager.tree.createNode(monitor.nodeValue, NODE_TYPES.WINDOW, terminal1);
      const nodeWindow2 = windowManager.tree.createNode(monitor.nodeValue, NODE_TYPES.WINDOW, terminal2);
