...
describe('Bug #305: Resize boundary behavior', () => {
  ...
  describe('_normalizeSiblingPercents helper', () => {
    it('should normalize sibling percentages to sum to 1.0', () => {
      ...
      const nodeWindow1 = windowManager.tree.createNode(monitor.nodeValue, NODE_TYPES.WINDOW, window1);
      const nodeWindow2 = windowManager.tree.createNode(monitor.nodeValue, NODE_TYPES.WINDOW, window2);
      const nodeWindow3 = windowManager.tree.createNode(monitor.nodeValue, NODE_TYPES.WINDOW, window3);
