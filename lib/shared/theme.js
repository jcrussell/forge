...
export class ThemeManagerBase extends GObject.Object {
  ...
  constructor({ configMgr, settings }) {
    ...
    // The tagging approach works by comparing this value against stored settings.
    // When cssTag > stored value, patchCss() updates user config to match new defaults.
    // Note: Future improvements could implement smarter CSS merging to preserve user customizations.
    // See patchCss() and _needUpdate() for the current implementation.
    this.cssTag = 38;
  ...
  patchCss() {
    if (this._needUpdate()) {
      ...
      // Bug #312 (forge-9sd): TARGET_DEFAULT_PERMS makes the copied config stylesheet
      // use the umask default (writable) instead of inheriting the read-only install-dir
      // perms, which would otherwise make later color writes fail silently.
