...
/** @typedef {import('gi://Gio').default.Settings} GioSettings */
...
export class ConfigSync extends GObject.Object {
   ...
   * @param {GioSettings} params.settings
   * @param {GioSettings} params.kbdSettings
   */
  ...
  _configChangedExternally() {
    for (const name of /** @type {("settings" | "keybindings")[]} */ ([
      "settings",
      "keybindings",
    ])) {
  ...
  importAll() {
    ...
    // forge-q9e5: propagate the real import result. importSettings/importKeybindings
    // return false on a missing/corrupt file (or a set_* throw that aborts the
    // remaining keys), but the return was discarded and prefs always toasted
    // "imported successfully". Callers now get the truth.
    //
    // forge-7m2f: reported PER FILE. A merely-absent file reads as false, while
    // callers only require that ONE of the two exists, so a combined boolean
    // can't tell "half the config imported fine" from "the import failed".
    const settingsOk = this.importSettings();
    const keybindingsOk = this.importKeybindings();
    ...
    return { settings: settingsOk, keybindings: keybindingsOk };
  }
  ...
  exportSettings() {
    try {
      ...
      // forge-1n2v: propagate the write result — don't report success or stamp
      // config-last-export when the file was never written.
      if (!this._configMgr.saveSettingsProps(props)) {
        Logger.error("Failed to export settings: write failed");
        return false;
      }
  ...
  exportKeybindings() {
    try {
      ...
      // forge-1n2v: propagate the write result (see exportSettings).
      if (!this._configMgr.saveKeybindingsProps(props)) {
        Logger.error("Failed to export keybindings: write failed");
        return false;
      }
      this._settings.set_uint64("config-last-export", Math.floor(Date.now() / 1000));
   ...
   *
   * forge-7m2f: the two booleans used to be discarded, so every caller reported
   * success even when nothing was written (an unwritable ~/.config/forge/config).
   *
   * @returns {boolean} true when BOTH files were written.
   */
  exportAll() {
    const settingsOk = this.exportSettings();
    const keybindingsOk = this.exportKeybindings();
    this._recordWrittenMtimes();
    return settingsOk && keybindingsOk;
  }
   ...
   *
   * forge-7m2f: this used to flip configFilesLoaded and config-file-sync-enabled
   * unconditionally, so a failed export left the extension claiming a portable
   * config existed when no file had been written.
   *
   * @returns {boolean} true when the export succeeded and sync was enabled.
   */
  enablePortableConfig() {
    if (!this.exportAll()) {
      Logger.error("Portable config not enabled: export failed");
      return false;
    }
