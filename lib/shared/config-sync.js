...
export class ConfigSync extends GObject.Object {
  ...
  importSettings() {
    ...
    try {
      ...
      if (props.behavior) {
        for (const key of SETTINGS_KEYS.behavior) {
          if (props.behavior[key] !== undefined) {
            this._setSettingValue(key, props.behavior[key]);
          }
        }
      }
      ...
      if (props.appearance) {
        for (const key of SETTINGS_KEYS.appearance) {
          if (props.appearance[key] !== undefined) {
            this._setSettingValue(key, props.appearance[key]);
          }
        }
      }
      ...
      if (props.workspaces) {
        for (const key of SETTINGS_KEYS.workspaces) {
          if (props.workspaces[key] !== undefined) {
            this._setSettingValue(key, props.workspaces[key]);
          }
        }
      }
      ...
      if (props.development) {
        for (const key of SETTINGS_KEYS.development) {
          if (props.development[key] !== undefined) {
            this._setSettingValue(key, props.development[key]);
          }
        }
      }
      ...
      if (props.other) {
        for (const key of SETTINGS_KEYS.other) {
          if (props.other[key] !== undefined) {
            this._setSettingValue(key, props.other[key]);
          }
        }
      }
  ...
  exportSettings() {
    try {
      ...
      for (const key of SETTINGS_KEYS.behavior) {
        props.behavior[key] = this._getSettingValue(key);
      }

      // Export appearance settings
      for (const key of SETTINGS_KEYS.appearance) {
        props.appearance[key] = this._getSettingValue(key);
      }

      // Export workspace settings
      for (const key of SETTINGS_KEYS.workspaces) {
        props.workspaces[key] = this._getSettingValue(key);
      }

      // Export development settings
      for (const key of SETTINGS_KEYS.development) {
        props.development[key] = this._getSettingValue(key);
      }

      // Export other settings
      for (const key of SETTINGS_KEYS.other) {
        props.other[key] = this._getSettingValue(key);
      }
