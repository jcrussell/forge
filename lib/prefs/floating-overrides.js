...
/**
 * True when an structurally identical override is already in the list.
 *
 * forge-eryk: the Add button pushed unconditionally, so the same rule could be
 * added twice — and removeOverride is remove-ALL by design (that is what keeps
 * same-class siblings safe, forge-fov0), so trashing either row deleted both
 * while the other row was still on screen. The list must never hold duplicates.
 *
 * @param {Array<object>} overrides - the current overrides
 * @param {object} candidate - the override about to be added
 * @returns {boolean}
 */
export function hasOverride(overrides, candidate) {
  return overrides.some((o) => sameOverride(o, candidate));
}
