...
export class WindowManager extends GObject.Object {
  ...
  _normalizeSiblingPercents(parentNode) {
    ...
    if (!parentNode.isStacked || !parentNode.isTabbed) return;
    if (parentNode.isStacked() || parentNode.isTabbed()) return;
  ...
  moveWindowToPointer(focusNodeWindow, preview = false) {
    ...
    if (nodeWinAtPointer) {
      ...
      if (isCenter) {
        ...
        if (stackedOrTabbed) {
          ...
          if (!isMonParent) {
            ...
            // Monitor is the stacked/tabbed container - prepend window
            containerNode = parentNodeTarget;
            referenceNode = parentNodeTarget.firstChild;
          }
        } else {
          if (horizontal) {
            containerNode = parentNodeTarget;
        ...
        if (stackedOrTabbed) {
          ...
          if (!isMonParent) {
            ...
            // Monitor is the stacked/tabbed container - append window
            containerNode = parentNodeTarget;
            referenceNode = null; // Append at end
          }
        } else {
          if (horizontal) {
            containerNode = parentNodeTarget;
      ...
      if (!isCenter) {
        // All edge drops (left/right/top/bottom) now detach from stacked/tabbed
        // containers to create a proper split, so always show tiled preview
  ...
  _handleResizing(focusNodeWindow) {
    ...
    for (let grabOp of grabOps) {
      ...
      if (
        sameParent &&
        parentNodeForFocus &&
        (parentNodeForFocus.isTabbed() || parentNodeForFocus.isStacked())
      ) {
        sameParent = false;
      }
