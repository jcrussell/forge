...
export class WindowManager extends GObject.Object {
  ...
  _bindSignals() {
    ...
    this._workspaceManagerSignals = [
      ...
      globalWsm.connect("workspaces-reordered", () => this.reloadTree("workspaces-reordered")),
  ...
  showWindowBorders() {
    ...
    let monitorNode = this.tree.findAncestorMonitor(nodeWindow);
    let tiledChildren = this._tiledWindowsOnMonitor(monitorNode);
    let inset = 3;
    ...
    const splitRadius = this.ext.settings.get_uint("focus-border-radius") * Utils.dpi();
  ...
  calculateGaps(node) {
    ...
    if (!node.isRoot()) {
      ...
      let parentNode = this.tree.findAncestorMonitor(node);
      if (parentNode) {
        let tiled = this._tiledWindowsOnMonitor(parentNode);
                ...
                if (this.floatingWindow(focusNodeWindow)) {
                  this.queueEvent({
                    ...
                    callback: () => {
                      ...
                      if (fw && this.floatingWindow(this.tree.findNode(fw))) fw.raise();
  ...
  _focusWindowUnderPointer() {
    ...
    if (!this.shouldFocusOnHover || this.disabled) {
  ...
  isFloatingExempt(metaWindow) {
    ...
    let hasSpecificTileOverride = false;
    let hasClassOnlyTileOverride = false;
    for (const override of allOverrides) {
      ...
      if (override.wmTitle || override.wmId) {
        hasSpecificTileOverride = true;
      } else {
        hasClassOnlyTileOverride = true;
      }
    }
    ...
    let floatByRole =
      ...
      // forge-yyum: a window pinned "Always on Visible Workspace" is shown on
      // every workspace by Mutter, but the tree can only home it under one
      // workspace's container — leaving it tiled churns layouts as it is
      // re-homed onto each active workspace. Treat it as an overlay and float it,
      // mirroring the Always-on-Top rule above; un-pinning re-tiles it.
    ...
    if (hasSpecificTileOverride) return false;
    if (hasClassOnlyTileOverride && !floatByRole) return false;
