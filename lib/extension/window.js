...
import { calculateDropRegions, detectDropZone, DROP_ZONES } from "./utils.js";
...
export class WindowManager extends GObject.Object {
  ...
  addFloatOverride(metaWindow, withWmId) {
    ...
    for (let override of overrides) {
      ...
      // Bug #172 fix: More precise duplicate detection for per-window vs class-based
      if (withWmId) {
        // Per-window override - only skip if exact wmId match exists
        if (override.wmClass === wmClass && override.wmId === wmId && override.mode === "float")
          return;
      } else {
        // Class-based override - skip if class-based (no wmId) exists
        if (
  ...
  toggleFloatingMode(action, metaWindow) {
    ...
    if (floatingExempt) {
      ...
      if (!this.isActiveWindowWorkspaceTiled(metaWindow)) {
        // Bug #319 fix: Use float setter to properly handle _forgeSetAbove
  ...
  _removeSignals() {
    ...
    for (let i = 0; i < numberOfWorkspaces; i++) {
      let workspace = globalWsm.get_workspace_by_index(i);
      disconnectSignals(workspace, workspace.workspaceSignals);
  ...
  _restoreFocusAfterWindowClosed(closedNodeWindow) {
    ...
    const workspaceWindows = currentWs
      .list_windows()
      .filter((w) => !w.minimized && w.get_window_type() === Meta.WindowType.NORMAL);
  ...
  _executeDropOperation(focusNodeWindow, operation, nodeWinAtPointer, ctx) {
    const { containerNode, referenceNode, zone } = operation;
    ...
    const isBeforeEdge = zone === DROP_ZONES.LEFT || zone === DROP_ZONES.TOP;
    const isHorizontalEdge = zone === DROP_ZONES.LEFT || zone === DROP_ZONES.RIGHT;
    ...
    if (operation.isSwap) {
      ...
      const orientation = isHorizontalEdge
  ...
  _buildDropOperation(zone, ctx) {
    ...
    if (stackedOrTabbed) {
      ...
      if (!isMonParent) {
        referenceNode = isBeforeEdge ? parentNodeTarget : parentNodeTarget.nextSibling;
        containerNode = parentNodeTarget.parentNode;
      } else {
        containerNode = parentNodeTarget;
        referenceNode = isBeforeEdge ? parentNodeTarget.firstChild : null;
      }
      return {
        zone,
    ...
    // Normal container: determine if we need to create a new con
    // Create con when orientation doesn't match edge direction
    const needsCreateCon = isHorizontalEdge !== horizontal;

    const referenceNode = isBeforeEdge ? nodeWinAtPointer : nodeWinAtPointer.nextSibling;

    return {
      zone,
      shouldCreateCon: needsCreateCon,
      containerNode: parentNodeTarget,
      referenceNode,
      previewRect: previewRegions[zone.toLowerCase()],
      previewClass: "window-tilepreview-tiled",
  ...
  moveWindowToPointer(focusNodeWindow, preview = false) {
    ...
    const zone = detectDropZone(hoverRegions, this.getPointer());
  ...
  _handleResizing(focusNodeWindow) {
    ...
    for (let grabOp of grabOps) {
      ...
      if (sameParent && parentNodeForFocus && parentNodeForFocus.isStackedOrTabbed()) {
        sameParent = false;
      }
