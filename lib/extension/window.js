...
export class WindowManager extends GObject.Object {
  ...
  addFloatOverride(metaWindow, withWmId) {
    ...
    for (let override of overrides) {
      ...
      if (withWmId) {
        ...
        if (override.wmClass === wmClass && override.wmId === wmId && override.mode === "float") return;
      } else {
        // Class-based override - skip if class-based (no wmId) exists
        if (override.wmClass === wmClass && !override.wmId && override.mode === "float" && !override.wmTitle) return;
      }
    }
    overrides.push({
      wmClass: wmClass,
  ...
  _normalizeSiblingPercents(parentNode) {
    ...
    // Only normalize if there's significant drift from 1.0
      ...
      if (orientation === ORIENTATION_TYPES.HORIZONTAL) {
        if (sameParent) {
          ...
          if (resizePairForWindow) {
            ...
            while (candidatePair && (this.floatingWindow(candidatePair) || this.minimizedWindow(candidatePair))) {
              candidatePair = this.tree.nextVisible(candidatePair, direction);
            }
            if (candidatePair && !this.floatingWindow(candidatePair) && !this.minimizedWindow(candidatePair)) {
