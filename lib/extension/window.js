...
export class WindowManager extends GObject.Object {
  ...
  static _isMaximized(metaWindow) {
    try {
  ...
  static _isNotMaximized(metaWindow) {
    try {
      return !metaWindow.is_maximized();
    } catch (e) {
  ...
  static _safeUnmaximize(metaWindow) {
    try {
      // GNOME 49+
  ...
  _bindSignals() {
    ...
    this._windowManagerSignals = [
      shellWm.connect("minimize", () => {
        ...
        this._renderWithFreezeState("minimize");
      }),
      shellWm.connect("unminimize", () => {
        ...
        this._renderWithFreezeState("unminimize");
      }),
  ...
  showWindowBorders() {
    ...
    let maximized = () =>
      WindowManager._isMaximized(metaWindow) || metaWindow.is_fullscreen() || gap === 0;
  ...
  handleMaximizeOnSingle() {
    ...
    monitors.forEach((monitor) => {
      ...
      if (tiled.length === 1) {
        ...
        if (WindowManager._isNotMaximized(metaWindow)) {
  ...
  handleUnmaximizeForTiling(newNodeWindow) {
    ...
    windows.forEach((nodeWindow) => {
      ...
      if (WindowManager._isMaximized(metaWindow)) {
        WindowManager._safeUnmaximize(metaWindow);
      }
    });
  }
  ...
  updateMetaPositionSize(_metaWindow, from) {
    ...
    if (focusNodeWindow.grabMode && tilingModeEnabled) {
      ...
      if (WindowManager._isNotMaximized(focusMetaWindow)) {
        this.renderTree(from);
      }
    }
    this.updateBorderLayout();
  ...
  updateDecorationLayout() {
    ...
    let monWsNoMaxWindows = activeWsNode.getNodeByType(NODE_TYPES.MONITOR).filter((monitor) => {
      return (
        monitor
          .getNodeByType(NODE_TYPES.WINDOW)
          .filter((w) => WindowManager._isMaximized(w.nodeValue) || w.nodeValue.is_fullscreen())
          .length === 0
      );
    });
