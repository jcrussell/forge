...
/**
 * A window's compositor actor (Clutter.Actor) carrying Forge's per-window bookkeeping
 * expandos: the tiled/split border Bins and the actor-level signal-handler ids that
 * get_compositor_private() returns untyped. Cast the compositor actor to this at the
 * point of retrieval so the attached fields are visible.
 * @typedef {import('gi://Clutter').default.Actor & {
 *   border?: St.Bin,
 *   splitBorder?: St.Bin,
 *   actorSignals?: number[],
 * }} ForgeActor
 */
/**
 * A Meta.Window carrying Forge's window-level bookkeeping expandos: signal-handler
 * ids plus the stacked-raise debounce source id and transient-above marker.
 * @typedef {Meta.Window & {
 *   windowSignals?: number[],
 *   _forgeStackTimeoutId?: number,
 *   _forgeTransientAbove?: boolean,
 * }} ForgeMetaWindow
 */
/**
 * Meta.GrabOp members that exist on some supported GNOME versions but are absent
 * from the meta-13 (GNOME 45) typings. Accessing them through this keeps the
 * version probes type-checked without asserting the members are always present
 * (they resolve to undefined where the running Mutter lacks them).
 * @typedef {typeof Meta.GrabOp & { COMPOSITOR?: number, RESIZING_UNKNOWN?: number }} GrabOpCompat
 */
 ...
 * @param {number[] | null | undefined} signals - Array of signal IDs
 */
...
export class WindowManager extends GObject.Object {
  ...
  /** @type {Tree | null} */
  _tree;
  /** @type {number} GLib source id for the pointer-focus debounce (0 when idle); see FocusManager */
  _pointerFocusTimeoutId;
  /** @type {number} GLib source id for the workspace window-add debounce (0 when idle); see WorkspaceManager */
  _wsWindowAddSrcId;
  ...
  constructor(ext) {
    ...
    this.shouldFocusOnHover = this.ext.settings?.get_boolean("focus-on-hover-enabled") ?? false;
  ...
  _updateWindowOverrides(updateFn, metaWindow, withWmId) {
    ...
    if (!configMgr) return;
    let currentProps = configMgr.windowProps;
  ...
  _reconcileFullscreenFloatDemotion() {
    ...
    nodes.forEach((w) => {
      const metaWindow = w.nodeValue;
      // forge-olv3: windowDestroy calls this synchronously pre-prune, so a
      // finalized wrapper's is_fullscreen() would throw and abort the destroy.
      if (!Utils.isWindowAlive(metaWindow) || !metaWindow.is_fullscreen()) return;
    ...
    this._withSuppressedAboveHandler(() => {
      nodes.forEach((w) => {
        if (w.mode !== WINDOW_MODES.FLOAT) return;
        const metaWindow = w.nodeValue;
        // forge-olv3: skip finalized wrappers (is_above/is_fullscreen would throw
        // out of the synchronous windowDestroy path), mirroring _forEachFloatNode.
        if (!Utils.isWindowAlive(metaWindow)) return;
  ...
  trackCurrentMonWs() {
    ...
    // A focused window can transiently report no workspace (creation, Wayland
    // race). get_workspace().index() would throw on null — bail like the
    // !currentWsNode guard below; tracking re-runs on the next focus.
    const activeWorkspace = metaWindow.get_workspace();
    ...
    const monWindows = currentWsNode.getNodeByType(NODE_TYPES.WORKSPACE).flatMap((ws) => {
      return ws
        ...
        .filter(
          (w) =>
            // forge-0qx7: runs from raw signal handlers before pruneDeadWindows,
            // so a finalized wrapper's .minimized would throw and abort tracking.
            Utils.isWindowAlive(w.nodeValue) &&
  ...
  resize(grabOp, amount) {
    ...
    // forge-h6z9's cross-window flush now lives at the top of _handleGrabOpBegin
    // (forge-zr6f), which this calls immediately below — so it covers this path
    // (focus drifted between two keyboard resizes) and the pointer-grab path
    // alike. metaWindow is this.focusMetaWindow, the same key it compares on.
    ...
    this.move(metaWindow, rect, null, { skipOffscreenClamp: true, commitDuringGrab: true });
  ...
  determineSplitLayout() {
    ...
    const monitorRect = Utils.getMonitorGeometrySafe(global.display.get_current_monitor());
    if (!monitorRect) return LAYOUT_TYPES.HSPLIT; // sensible default when geometry unavailable
    return monitorRect.width < monitorRect.height ? LAYOUT_TYPES.VSPLIT : LAYOUT_TYPES.HSPLIT;
  }
  ...
  applyDefaultLayoutToContainer(container) {
    ...
    if (!settings) return;
    const defaultLayout = settings.get_string("default-window-layout");
    if (defaultLayout === "tabbed" && settings.get_boolean("tabbed-tiling-mode-enabled")) {
      container.layout = LAYOUT_TYPES.TABBED;
    } else if (defaultLayout === "stacked" && settings.get_boolean("stacked-tiling-mode-enabled")) {
  ...
  hideWindowBorders() {
    return this.decorationManager.hideWindowBorders();
  }
  ...
  move(
    ...
    rect,
    workArea = null,
    { skipOffscreenClamp = false, commitDuringGrab = false } = {}
  ) {
    if (!metaWindow) return;
    // forge-r2k9: don't reposition a window under a live interactive grab — an
    // out-of-band render (tree.apply @ tree.js) would teleport it mid-drag/resize.
    // The old `metaWindow.grabbed` guard tested a Meta.Window property Forge never
    // sets, so it never fired; use Forge's own grab state (grabMode /
    // _draggedNodeWindow, set in _handleGrabOpBegin) instead. Interactive grabs
    // reposition via move_frame() directly, not through move(), so this doesn't
    // fight the gesture.
    // forge-t7qh: the synchronous keyboard-resize path (resize() @ this file) sets
    // grabMode itself and then relies on THIS move() to commit the frame and fire
    // size-changed. That is an in-band commit, not an out-of-band teleport, so it
    // opts out via commitDuringGrab; without it, keyboard resize is a total no-op.
    const grabbedNode = this.findNodeWindow(metaWindow);
    if (
      !commitDuringGrab &&
      grabbedNode &&
      (grabbedNode.grabMode || grabbedNode === this._draggedNodeWindow)
    )
      return;
    ...
    let windowActor = /** @type {ForgeActor} */ (metaWindow.get_compositor_private());
  ...
  _removeSignals() {
    ...
    const allWindows = /** @type {Set<ForgeMetaWindow>} */ (
      new Set(this.windowsAllWorkspaces || [])
    );
    ...
    for (const metaWindow of allWindows) {
      ...
      // bug-328: a window finalized (closed) while still referenced by a tree
      // WINDOW node throws on EVERY method — not just disconnect(). Probing
      // get_compositor_private() on such a wrapper would throw here, OUTSIDE
      // disconnectSignals' try/catch, aborting cleanup for every remaining
      // window (the g_signal_handler_disconnect storm this pass hardens against).
      // A finalized window's actor died with it, so there is nothing to clean.
      let windowActor = Utils.isWindowAlive(metaWindow)
        ? /** @type {ForgeActor} */ (metaWindow.get_compositor_private())
        : null;
      if (windowActor) {
    ...
    if (this._freezeRender || !this.ext.settings.get_boolean("tiling-mode-enabled")) {
      ...
      if (!this._renderTreeSrcId) {
        this._renderTreeSrcId = GLib.idle_add(GLib.PRIORITY_DEFAULT, () => {
          try {
            // forge-opsl: re-check the freeze at FIRE time, not just call time. A
            // force render scheduled while unfrozen must not reflow the tree if a
            // later path (e.g. grab-op-begin) froze after scheduling — that reflow
            // is exactly what the freeze exists to prevent. The finally still resets
            // the src id and re-freezes only when THIS call owned the freeze.
            if (this._freezeRender) return false;
  ...
  sameParentMonitor(firstNode, secondNode) {
    ...
    // get_workspace() throws on a finalized wrapper, so probe liveness before
    // dereferencing (matches swapPairs, the caller that runs right after us).
    if (!Utils.isWindowAlive(firstNode.nodeValue)) return false;
    if (!Utils.isWindowAlive(secondNode.nodeValue)) return false;
    const firstWs = firstNode.nodeValue.get_workspace();
    const secondWs = secondNode.nodeValue.get_workspace();
    if (!firstWs || !secondWs) return false;
  ...
  showWindowBorders() {
    return this.decorationManager.showWindowBorders();
  }

  updateBorderLayout() {
    return this.decorationManager.updateBorderLayout();
  }
  ...
  _tiledWindowsOnMonitor(monitorNode) {
    ...
    return monitorNode
      ...
      .filter((t) => t.isWindow() && Utils.isWindowAlive(t.nodeValue) && !t.nodeValue.minimized);
  }
  ...
  handleUnmaximizeForTiling(newNodeWindow) {
    ...
    windows.forEach((nodeWindow) => {
      ...
      // forge-z9ky: pre-prune path (trackWindow → window-created); a finalized
      // wrapper is truthy so `.minimized` would throw. isWindowAlive null-checks.
      if (!Utils.isWindowAlive(metaWindow) || metaWindow.minimized) return;
  ...
  trackWindow(_display, metaWindow) {
    ...
    if (this._validWindow(metaWindow)) {
      ...
      if (!existNodeWindow) {
        ...
        if (settings.get_boolean("auto-split-enabled") && this.focusMetaWindow) {
        ...
        const placement = settings.get_string("new-window-placement");
   ...
   * @param {ForgeMetaWindow} metaWindow
   * @param {ForgeActor} windowActor
   */
  ...
  postProcessWindow(nodeWindow) {
    ...
    if (metaWindow) {
      if (metaWindow.get_title() === this.prefsTitle) {
        // A just-mapped window can report no workspace; guard the deref so the
        // prefs-window centering below still runs.
        const workspace = metaWindow.get_workspace();
        if (workspace) {
          workspace.activate_with_focus(metaWindow, global.display.get_current_time());
        }
  ...
  _isInSkipList(settingKey, value) {
    if (!this.ext.settings) return false;
  ...
  _restoreFocusAfterWindowClosed(restore) {
    ...
    const activate = (metaWindow) => {
      // forge-dvce: runs synchronously inside windowDestroy pre-prune, so a
      // finalized sibling wrapper's .minimized/raise() would throw and abort the
      // rest of the destroy (queued render never enqueued).
      if (!Utils.isWindowAlive(metaWindow) || metaWindow.minimized) return false;
      metaWindow.raise();
    ...
    if (!wsNode) return;
    const candidates = wsNode.getNodeByType(NODE_TYPES.WINDOW).filter(
      (node) =>
        node !== restore.closedNodeWindow &&
        // forge-dvce: guard get_window_type() against finalized wrappers.
        Utils.isWindowAlive(node.nodeValue) &&
  ...
  updateDecorationLayout() {
    return this.decorationManager.updateDecorationLayout();
  }
  ...
  /**
   * @param {string} reason
   * @param {{hideBorders?: boolean, resetGrandparentIfEmpty?: boolean, metaWindow?: Meta.Window | null}} [opts]
   */
  _onMinimizeChange(
    reason,
  ...
  minimizedWindow(node) {
    if (!node) return false;
    // forge-3cfd: truthiness ≠ liveness; a finalized wrapper throws on `.minimized`
    // in the _handleResizing pair-walk (pre-prune). isWindowAlive null-checks.
    return (
      node._type === NODE_TYPES.WINDOW && Utils.isWindowAlive(node._data) && node._data.minimized
    );
  }
  ...
  moveWindowToPointer(focusNodeWindow, preview = false) {
    ...
    // forge-o09u: nodeWinAtPointer is cached by the last motion; a fast Wayland
    // close can finalize its wrapper before the drop, so probe liveness before
    // get_frame_rect() below (mirrors the preview path, forge-xom3).
    if (
      !nodeWinAtPointer.nodeValue ||
      !nodeWinAtPointer.parentNode ||
      !Utils.isWindowAlive(nodeWinAtPointer.nodeValue)
    ) {
    ...
    const ctx = {
      ...
      centerLayout: (settings.get_string("dnd-center-layout") ?? "").toUpperCase(),
   ...
   *
   * @param {[number, number, ...unknown[]]} pointer x and y coordinates (extra
   *   trailing values from global.get_pointer(), e.g. the modifier mask, are ignored)
  ...
  _getMetaWindowAtPointer(pointer) {
    ...
    for (let i = windows.length - 1; i >= 0; i--) {
      ...
      // forge-h7fb: get_window_actors() is display-wide, and neither a minimized
      // window nor one on another workspace loses its frame rect or its stacking
      // slot. Such a window would match the pointer first and shadow the real
      // hover target — _focusWindowUnderPointer then focuses something invisible
      // and returns, killing focus-on-hover for that region. isWindowAlive first:
      // this runs in a 16ms timeout whose GSource is removed if we throw, and
      // these accessors throw on a finalized wrapper (forge-e5mh).
      if (!Utils.isWindowAlive(metaWindow)) continue;
      if (metaWindow.minimized) continue;
      if (activeWorkspace && !metaWindow.located_on_workspace(activeWorkspace)) continue;
  ...
  _handleGrabOpBegin(_display, _metaWindow, grabOp) {
    // forge-zr6f: if the pending debounced end belongs to a DIFFERENT window than
    // the one this grab is for, flush that window's grab before dropping the timer.
    // forge-h6z9 fixed keyboard-resize -> keyboard-resize (via the same guarded
    // block in resize()) and end-fires-into-new-grab, but not the cross product:
    // keyboard-resize A, then a real pointer grab on B inside the 120ms window.
    // The unconditional clear below then stranded A with grabMode=RESIZING forever,
    // and since forge-r2k9 move() skips any node with grabMode, A was silently
    // excluded from every tree.apply() until it was grabbed again.
    //
    // MUST stay conditional: resize() calls this on every key auto-repeat, and an
    // unconditional _grabCleanup would null initRect/pairInitRects mid-hold and
    // re-anchor to the already-grown frame (the forge-12f/forge-5v6 breakage that
    // bug-532-held-resize pins). Same-window repeats take the false branch.
    // MUST stay above the tabbed-ancestor snapshot below, so cleaning A cannot
    // null a container initRect that this grab is about to take.
    if (
      this._manualResizeEndId &&
      this._manualResizeEndWindow &&
      this._manualResizeEndWindow !== this.focusMetaWindow
    ) {
  ...
  _handleGrabOpEnd(_display, _metaWindow, grabOp) {
    ...
    if (focusNodeWindow) {
      ...
      if (
        ...
        grabOp === /** @type {GrabOpCompat} */ (Meta.GrabOp).COMPOSITOR ||
  ...
  _resizeContainerAgainstSibling(container, grabbedWindow, currentRect, orientation, direction) {
    ...
    // forge-yz41: anchor BOTH the container's start slice and the pair on their
    // grab-begin rects, snapshotted on the grabbed window like the same-parent
    // branch (forge-12f). Reading live rects here re-debits the cumulative
    // changePx across X11's multiple size-changed events per move and drifts the
    // shared edge past the pointer.
    const startRect = container.initRect || this._pairInitRect(grabbedWindow, container);
    const pairRect = this._pairInitRect(grabbedWindow, pair);
    const parentRect = parent.rect;
  ...
  _handleResizing(focusNodeWindow) {
    ...
    for (let grabOp of grabOps) {
      ...
      if (initGrabOp === /** @type {GrabOpCompat} */ (Meta.GrabOp).RESIZING_UNKNOWN) {
      ...
      if (
        ...
        if (sameParent) {
          ...
          if (resizePairForWindow && resizePairForWindow.parentNode) {
            ...
            // (forge-34c6). forge-b602: the .parentNode.rect read (and the
            // _normalizeSiblingPercents(parentNodeForFocus.parentNode) call) below
            // dereference parentNode too, so guard both before dereferencing.
            if (!parentNodeForFocus || !parentNodeForFocus.parentNode) return;
            // forge-yz41: anchor both sides on their grab-begin rects (snapshotted
            // on the focus node like the same-parent branch, forge-12f). Live rects
            // re-debit the cumulative changePx across X11's multiple size-changed
            // events per move and drift the boundary.
            firstRect = this._pairInitRect(focusNodeWindow, parentNodeForFocus);
            secondRect = this._pairInitRect(focusNodeWindow, resizePairForWindow);
            if (!firstRect || !secondRect) {
              return;
            }
  ...
  _getDragDropCenterPreviewStyle() {
    const settings = this.ext.settings;
    if (!settings) return "window-tilepreview-";
    const centerLayout = settings.get_string("dnd-center-layout");
    ...
    if (freshProps) {
      ...
      if (pruneStaleWmId) {
        ...
        if (this.windowProps.overrides.length !== beforeLength) {
          configMgr.windowProps = this.windowProps;
        }
      }
