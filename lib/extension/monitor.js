...
export class MonitorManager extends GObject.Object {
  ...
  addMonitor(wsIndex) {
    ...
    for (let mi = 0; mi < monitors; mi++) {
      ...
      // Bug #311 (forge-5ng): derive the default split from THIS monitor's
      // geometry, not the focused monitor's. Otherwise a rotated/portrait
      // monitor inherits the focused monitor's orientation (e.g. HSPLIT).
