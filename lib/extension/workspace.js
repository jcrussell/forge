      ...
      metaWorkspace.connect("window-added", (_, metaWindow) => {
        if (!this._extWm._wsWindowAddSrcId) {
          this._extWm._wsWindowAddSrcId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 200, () => {
            // Reset in finally: a window destroyed within the debounce throws on
            // the finalized wrapper, and a skipped reset would wedge this guard
            // for the whole session (same class as the forge-cuv renderTree fix).
            try {
