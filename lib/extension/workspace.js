...
export class WorkspaceManager extends GObject.Object {
  ...
  _renumberWorkspaces({ shouldRenumber, compareFn, newIndex }) {
    ...
    // The previous in-place delete(oldIndex)+set(newIndex) was collision-safe
    // only by virtue of the callers' sort order; capturing the arrays up front
    // and deleting all sources before any set makes the rekey independent of
    // processing order, so it can never overwrite a live signal array.
    const signalMoves = [];
    for (const { node: wsNode, oldIndex } of toRenumber) {
      ...
      if (this._workspaceSignals.has(oldIndex)) {
        signalMoves.push({ oldIndex, idx, signals: this._workspaceSignals.get(oldIndex) });
      }
    }
    ...
    for (const { idx, signals } of signalMoves) {
      this._workspaceSignals.set(idx, signals);
    }
  }
