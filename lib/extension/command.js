...
export class CommandHandler extends GObject.Object {
  ...
  execute(action) {
    ...
    // The `if (handler)` guard preserves the old switch's `default: break` no-op
    // for unknown actions.
  ...
  _buildHandlers() {
    ...
    this._handlers = {
      ...
      SnapLayoutMove: (action, ctx) => {
        const { wm, focusNodeWindow } = ctx;
        // forge-9s8c: guard-clause at the top and an unconditional return at the
        // handler's outer scope. Previously the whole body (including its break) sat
