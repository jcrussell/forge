...
export class CommandHandler extends GObject.Object {
  ...
  execute(action) {
    ...
    switch (action.name) {
      ...
      case "SnapLayoutMove": {
        // forge-9s8c: guard-clause at the top and an unconditional break at the
        // case's outer scope. Previously the whole body (including its break) sat
        // inside `if (focusNodeWindow)`, so a falsy focus fell through into
        // ShowTabDecorationToggle — masked today only because that case
        // early-returns, but one edit away from silently toggling a GSetting.
        if (!focusNodeWindow) break;
        let workareaRect = Utils.getWorkAreaSafe(focusNodeWindow.nodeValue);
        if (!workareaRect) break;
      ...
      case "WindowExpand":
        wm.expand(action.amount);
        break;

      case "WindowShrink":
        wm.shrink(action.amount);
        break;
