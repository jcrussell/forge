  ...
  _buildHandlers() {
    ...
    this._handlers = {
      ...
      WorkspaceActiveTileToggle: (action, ctx) => {
        ...
        // Trim entries: membership (_isWorkspaceSkipped) trims, so a spaced value
        // like "0, 1, 2" must be trimmed here too or indexOf misses and splice(-1)
        // deletes the wrong (last) entry (forge-myzp).
        let skippedArr =
          skippedWorkspaces.length === 0 ? [] : skippedWorkspaces.split(",").map((s) => s.trim());
      ...
      LayoutStackedToggle: (action, ctx) => {
        ...
        if (focusNodeWindow.parentNode.isMonitor()) {
          // split() returns null for a FLOAT node; stamping layout onto the
          // pre-existing MONITOR parent would stick STACKED on the monitor (forge-l4pd).
          if (!wm.tree.split(focusNodeWindow, ORIENTATION_TYPES.HORIZONTAL, true)) return;
        }
      ...
      LayoutTabbedToggle: (action, ctx) => {
        ...
        if (focusNodeWindow.parentNode.isMonitor()) {
          // split() returns null for a FLOAT node; stamping layout onto the
          // pre-existing MONITOR parent would stick TABBED on the monitor (forge-l4pd).
          if (!wm.tree.split(focusNodeWindow, ORIENTATION_TYPES.HORIZONTAL, true)) return;
        }
      ...
      PrefsOpen: (action, ctx) => {
        ...
        // get_workspace() returns null on an unmanaged window; capture once and
        // guard instead of re-calling it to deref (no-unguarded-window-deref).
        const existWorkspace = existWindow && existWindow.get_workspace();
        if (existWindow && existWorkspace) {
          existWorkspace.activate_with_focus(existWindow, global.display.get_current_time());
      ...
      ConfigReload: (action, ctx) => {
        ...
        if (wm.ext.configSync) {
          // forge-7m2f: report what actually imported instead of always claiming
          // success. A file that simply isn't there also reads as false, so the
          // honest line here is "did anything load at all".
          const result = wm.ext.configSync.importAll();
          if (result.settings || result.keybindings) {
            Logger.info("Configuration and portable settings reloaded from files");
          } else {
            Logger.error("Configuration reload: no portable config file could be read");
          }
        } else {
      ...
      ConfigExport: (action, ctx) => {
        const { wm } = ctx;
        if (wm.ext.configSync) {
          // forge-7m2f: don't log success when nothing was written.
          if (wm.ext.configSync.enablePortableConfig()) {
            Logger.info("Configuration exported to portable config files");
          } else {
            Logger.error("Configuration export to portable config files failed");
          }
        }
      },
