...
const KEY_DESCRIPTIONS = {
  ...
  "window-focus-next": "Focus next window (cyclic)",
  "window-focus-prev": "Focus previous window (cyclic)",
  ...
  "window-swap-next": "Swap next window (cyclic)",
  "window-swap-prev": "Swap previous window (cyclic)",
  ...
  "window-golden-ratio": "Golden ratio resize",
...
export class Cheatsheet extends GObject.Object {
  ...
  _buildOverlay() {
    ...
    for (const [categoryName, bindings] of groups) {
      ...
      // Create category section and add to column
  ...
  _createCategorySection(categoryName, bindings) {
    // Section container with bottom margin for spacing between categories
    const section = new St.BoxLayout({
      vertical: true,
      style: "margin-bottom: 20px;",
    });

    // Category header with subtle background, centered
  ...
  _getGroupedKeybindings() {
    ...
    const orderedGroups = [];
    const categoryOrder = [...new Set(CATEGORIES.map((c) => c.name)), "Other"];
