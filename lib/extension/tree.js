...
export class Node extends GObject.Object {
  ...
  removeChild(node) {
    ...
    // A removed node may own a header tab in its parent's decoration: CON nodes
    // via _ensureConTab (forge-37r), WINDOW nodes via _createWindowTab
    // (forge-wrot — every window in a stacked/tabbed CON). Tear it down so the
    // St.BoxLayout + its buttons/signals don't leak. _destroyTab no-ops when
    // there is no tab, so a single guard covers both node types (DRY).
    if (node.tab) {
      node._destroyTab();
    }
  ...
  _buildTabBase(app, labelText) {
    ...
    iconBin.child = app.create_icon_texture(24 * Utils.dpi());
...
export class Tree extends Node {
   ...
   */
  move(node, direction) {
    // Skip move for floating or minimized windows
    if (!node) return false;
    if (node.isFloat()) {
      ...
      case NODE_TYPES.MONITOR:
        ...
        if (
          ...
          this.extWm.move(
            node.nodeValue,
            rect,
            node.nodeValue.get_work_area_for_monitor(targetMonitor)
          );
          this.extWm.movePointerWith(node);
        } else if (ownMonNode) {
          if (position === POSITION.AFTER) {
    ...
    while (flattened) {
      ...
      for (const parent of nestedCons) {
        ...
        for (const grandchild of grandchildren) {
          ...
          // destroyed wholesale by removeChild below; drop the dangling reference
          // (else node.tab points at a deallocated actor and the next render
          // throws on it). CON header tabs regenerate via _ensureConTab each
          // render once _tabRep is cleared.
          const hadTab = wasStackedOrTabbed && grandchild.tab;
          if (hadTab) {
            grandchild.tab = null;
            grandchild._tabRep = null;
          }
          // Reparent BEFORE rebuilding the tab. appendChild detaches via
          // removeChild, which now tears down a WINDOW node's tab too
          // (forge-wrot); nulling above keeps that detach a no-op, and rebuilding
          // afterwards means the fresh tab survives the reparent and is parented
          // under the new container on the next render (forge-wrot/gdsz).
          parent.appendChild(grandchild);
          if (hadTab && grandchild.isWindow()) grandchild._createWindowTab();
        }
  ...
  processGap(node) {
    ...
    if (nodeWidth > gap * 2 && nodeHeight > gap * 2) {
      ...
      // Note: Gap calculation applies uniform padding. Future improvements could:
      // - Detect adjacent windows and share gap space between them
      // - Adjust gaps based on display scaling factor
      // See: https://github.com/forge-ext/forge for feature requests
      nodeWidth -= gap * 2;
      nodeHeight -= gap * 2;
    }
    return { x: nodeX, y: nodeY, width: nodeWidth, height: nodeHeight };
  ...
  processStacked(node, child, params, _index) {
    ...
    // Lay the per-child header tabs out as a vertical column. St.BoxLayout.vertical
    // was deprecated on GNOME 48/49 and removed on GNOME 50; use orientation.
    node.decoration.orientation = Clutter.Orientation.VERTICAL;
    ...
    // Every child gets the same area below the title-bar column; the focused one is
    // raised on top (focus handling lives in getFocusOnAttach/findNextFocus).
    child.rect = {
      x: nodeRect.x,
      y: nodeRect.y + cappedBars,
    ...
    if (node.decoration && (child.isWindow() || child.isCon())) {
      ...
      if (gap === 0) adjustY = renderRect.y;

      let decoration = node.decoration;
      if (decoration !== null && decoration !== undefined) {
        try {
          decoration.set_size(adjustWidth, totalBars);
          ...
          // Stack the tabs vertically: each shares the column height equally.
          if (child.tab && !decoration.contains(child.tab)) {
            child.tab.y_expand = true;
            decoration.add_child(child.tab);
          }
        } catch (e) {
          Logger.warn(`Failed to update stacked decoration: ${e}`);
          ...
          // otherwise stay parented in window_group forever (forge-ogmd). A
          // second throw on a finalized actor must not escape the render loop.
  ...
  processTabbed(node, child, params, _index) {
    // forge-s7qo: the catch below nulls node.decoration to "recreate on next
    // render", but _createDecoration is otherwise only called from the Node
    // constructor — without this self-heal the tab bar would be gone for the rest
    // of the session after a single throw. _createDecoration early-returns when a
    // decoration already exists, so this is a no-op on the normal path.
    if (!node.decoration) node._createDecoration();
    // Reset orientation to a horizontal row in case this container was just
    // toggled from stacked. St.BoxLayout.vertical was removed on GNOME 50.
    node.decoration.orientation = Clutter.Orientation.HORIZONTAL;
    ...
    if (layout === LAYOUT_TYPES.TABBED) {
      ...
      if (node.childNodes.length > 1 || alwaysShowDecorationTab) {
        ...
        if (node.decoration && (child.isWindow() || child.isCon())) {
          let gap = this.extWm.calculateGaps(node);
          let renderRect = this.processGap(node);
          // Only window actors carry a themed border; a CON header tab contributes
          // none (forge-37r) — guard so CON children don't throw on a null border.
          let borderWidth =
            child.isWindow() && child.actor && child.actor.border
              ? child.actor.border.get_theme_node().get_border_width(St.Side.TOP)
              : 0;
          ...
          if (decoration !== null && decoration !== undefined) {
            try {
              ...
              // Ensure tab is added to decoration. Reset y_expand in case this tab
              // came from a stacked layout (processStacked sets it for the vertical
              // column); the horizontal tab strip distributes on the x axis.
              if (child.tab && !decoration.contains(child.tab)) {
              ...
              // Destroy the orphaned header before nulling so the old
              // St.BoxLayout isn't stranded in window_group (forge-ogmd); the
              // next render recreates it via _createDecoration.
              node._destroyDecoration();
            }
          }
