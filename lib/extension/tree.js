...
export class Node extends GObject.Object {
  ...
  removeChild(node) {
    ...
    // decoration — tear it down so it does not dangle.
  ...
  _destroyTab() {
    if (!this.tab) return;
    // The tab may already have been destroyed via the parent decoration's
    // destroy_all_children() (removeChild) — guard like the window-tab paths do.
    if (!this.tab._destroyed) {
  ...
  set float(value) {
    if (this.isWindow()) {
      ...
      if (value) {
        ...
        // Dialogs/transients must always be on top, regardless of setting
        const isDialog =
        ...
        if (!metaWindow.is_above() && (floatAlwaysOnTop || isDialog) && !isFullscreen) {
...
export class Tree extends Node {
   ...
   * grouping and the windows come back as a flat split. Scope (KISS): flat
   * groups whose children are all WINDOW nodes — the canonical "stack/tab these
   * N windows" case (including the tab-all-on-monitor path). Nested-CON groups
   * are deferred (logged + skipped) rather than silently flattened. Windows are
   * keyed by their live Meta.Window object, which survives reload().
   */
  snapshotLayoutGroups() {
    ...
    for (const layout of [LAYOUT_TYPES.STACKED, LAYOUT_TYPES.TABBED]) {
      for (const con of this.getNodeByLayout(layout)) {
        ...
        if (!children.every((child) => child.isWindow())) {
          Logger.debug(`snapshotLayoutGroups: skipping nested ${layout} group (deferred)`);
          continue;
        }
        groups.push({
          windows: children.map((child) => child.nodeValue),
          layout,
          lastTabFocus: con.lastTabFocus ?? null,
        });
      }
    }
    return groups;
  ...
  /**
   * forge-bqa: re-apply the groupings captured by snapshotLayoutGroups() after
   * the tree has been rebuilt with the windows flat under their monitor. For
   * each group, resolve the surviving windows; when at least two of them landed
   * back under one common parent, wrap that cohort in a CON carrying the saved
   * layout (built directly, not via split() which only wraps a single node).
   */
  restoreLayoutGroups(snapshot) {
    if (!snapshot || snapshot.length === 0) return;
    for (const group of snapshot) {
      const winNodes = group.windows.map((win) => this.findNode(win)).filter((node) => node);
      ...
      con.parentNode = parent;
      parent.childNodes.splice(insertIndex, 0, con);
      // appendChild detaches each window from `parent` first, preserving order.
      cohort.forEach((node) => con.appendChild(node));
      con.layout = group.layout;
      if (group.layout === LAYOUT_TYPES.TABBED) {
        ...
        const focusSurvived = cohort.some((node) => node.nodeValue === group.lastTabFocus);
        con.lastTabFocus = focusSurvived ? group.lastTabFocus : cohort[0].nodeValue;
      }
  ...
  _selectFocusWindow(container, previous) {
    ...
    if (container.layout === LAYOUT_TYPES.STACKED) return container.lastChild;
    if (windows.length > 1) {
    ...
    if (metaWindow.minimized) {
      ...
      if (Meta.is_wayland_compositor && Meta.is_wayland_compositor()) {
        try {
          ...
          if (!wasAbove) {
            metaWindow.make_above();
            // Clear previous stacking fix timer if pending
            if (this._waylandStackingTimeoutId) {
              GLib.Source.remove(this._waylandStackingTimeoutId);
              this._waylandStackingTimeoutId = 0;
            }
            // Briefly keep it above to ensure stacking, then restore
            this._waylandStackingTimeoutId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 50, () => {
              this._waylandStackingTimeoutId = 0;
              try {
  ...
  move(node, direction) {
    ...
    if (!next || next === -1) {
      if (next === -1) {
        // Handle edge of monitor - append or prepend to current monitor workspace
