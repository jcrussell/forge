...
/**
 * A container decoration actor (St.BoxLayout) tagged with Forge's tree bookkeeping:
 * `type` marks it as a forge decoration and `parentNode` links back to its Node.
 * @typedef {St.BoxLayout & { type?: string, parentNode?: Node, orientation?: Clutter.Orientation }} ForgeContainerActor
 */
...
export class Node extends GObject.Object {
  ...
  /** @type {St.Bin} backing actor for MONITOR/WORKSPACE nodes (assigned post-construction) */
  actorBin;
  /** @type {import('gi://Gio').default.Settings} shared extension settings, threaded onto CON/ROOT nodes */
  settings;
  /** @type {Meta.Window | St.Bin | null} value of the last-focused child in a stacked/tabbed container */
  lastTabFocus;
  ...
  get nextSibling() {
    if (this.parentNode) {
      ...
      // index is null only when parentNode is null (already ruled out above).
      if (index !== null && this.parentNode.lastChild !== this) {
  ...
  get previousSibling() {
    if (this.parentNode) {
      const index = this.index;
      // index is null only when parentNode is null (already ruled out above).
      if (index !== null && this.parentNode.firstChild !== this) {
        return this.parentNode.childNodes[index - 1];
      }
    }
    return null;
  ...
  _createWindowTab() {
    ...
    // These handlers are bound to the tab actors' lifetime: iconBin/titleButton/
    // closeButton are children of tabContents, destroyed via _destroyTab() /
    // decoration teardown, so Clutter auto-disconnects them (forge-fhen.12).
    /* eslint-disable local/no-untracked-connect */
    ...
    /* eslint-enable local/no-untracked-connect */
    ...
    // freshly-rebuilt one.
    // eslint-disable-next-line local/no-untracked-connect -- destroy handler on the actor itself; fires once at teardown, freed with tabContents.
  ...
  _ensureConTab() {
    ...
    // Bound to the tab actors' lifetime (see _createWindowTab) — auto-disconnected
    // when tabContents is destroyed (forge-fhen.12).
    /* eslint-disable-next-line local/no-untracked-connect */
    iconBin.connect("clicked", clickFn);
    // eslint-disable-next-line local/no-untracked-connect
    titleButton.connect("clicked", clickFn);
  ...
  _createDecoration() {
    ...
    let decoration = /** @type {ForgeContainerActor} */ (new St.BoxLayout());
  ...
  render() {
    ...
    if (this.tab !== null && this.tab !== undefined) {
      let titleLabel = /** @type {St.Button} */ (this.tab.get_child_at_index(1));
...
export class Tree extends Node {
  ...
  constructor(extWm) {
    ...
    this._extWm = extWm;
    // The Tree is only constructed from enable() (via WindowManager), when
    // ext.settings is a live Gio.Settings; it is nulled only in disable() after
    // the tree has been dropped. Guard the assignment so `settings` keeps the
    // non-null contract every node read (this.settings.get_*) relies on.
    const settings = this.extWm.ext.settings;
  ...
  _selectFocusWindow(container, previous) {
    const windows = container
      ...
      .filter((w) => w.isTile() && Utils.isWindowAlive(w.nodeValue) && !w.nodeValue.minimized);
  ...
  _activateWindowNode(next, direction) {
    ...
    // forge-icm0: reachable from a Focus keybinding before the idle-only prune;
    // a finalized wrapper throws on `.minimized`/get_monitor below.
    if (!Utils.isWindowAlive(metaWindow)) return null;
  ...
  move(node, direction) {
    ...
    if (!next || next === -1) {
      if (next === -1) {
        ...
        if (ownMonNode) {
          ...
          // forge-bxng: when the node is already the monitor's direct child at
          // this edge, the reparent is a positional no-op — running _finishMove
          // would wipe the untouched siblings' user-set split percents. Skip the
          // epilogue for that no-op (qxqb's epilogue still runs for a genuine
          // reparent out of a nested container).
          const noopEdge =
            priorParent === ownMonNode &&
            (position === POSITION.AFTER
              ? node === ownMonNode.lastChild
              : node === ownMonNode.firstChild);
          ...
          if (!noopEdge) this._finishMove(priorParent, ownMonNode);
          return true;
        }
      }
      return false;
    ...
    switch (next.nodeType) {
      ...
      case NODE_TYPES.MONITOR:
        ...
        // forge-n0a7: reaching the MONITOR case means next() found no in-direction
        // sibling within the monitor, so the node is already at the monitor's edge
        // in the move direction — cross regardless of first/last position (a middle
        // child of a perpendicular split, e.g. VSPLIT monitor moving right, must
        // still cross instead of reparenting to the bottom of its own monitor).
        if (!next.contains(node) && ownMonNode) {
  ...
  nextMonitor(nodeWindow, position, orientation) {
    ...
    // directionFrom returns undefined only when position is neither BEFORE nor
    // AFTER; without a direction there is no neighbour monitor to resolve.
    if (monitorDirection === undefined) return null;
    let targetMonitor = -1;
    ...
    // A window can transiently report no workspace; get_workspace().index()
    // would throw on null. No workspace means no target monitor node to find.
    const workspace = nodeWindow.nodeValue.get_workspace();
    if (!workspace) return null;
    let monWs = Utils.createMonitorWorkspaceId(targetMonitor, workspace.index());
  ...
  _neighborMonitorByGeometry(currentMonitor, direction) {
    const cur = Utils.getMonitorGeometrySafe(currentMonitor);
    if (!cur) return -1;
    ...
    for (let i = 0; i < count; i++) {
      ...
      const c = Utils.getMonitorGeometrySafe(i);
      if (!c) continue;
  ...
  nextVisible(node, direction) {
    ...
    // forge-3cfd: this feeds the _handleResizing pair-walk (window.js:3409/3438)
    // synchronously pre-prune, so a finalized wrapper is truthy and `.minimized`
    // would throw here before minimizedWindow() is ever reached. Probe liveness;
    // a dead wrapper is skipped like a minimized one (walk past it).
    if (
      next &&
      next.nodeType === NODE_TYPES.WINDOW &&
      (!Utils.isWindowAlive(next.nodeValue) || next.nodeValue.minimized)
    ) {
  ...
  swapPairs(fromNode, toNode, focus = true) {
    ...
    if (parentForTo && parentForFrom) {
      ...
      // forge-2ngl: exchange modes only AFTER the rect null-guard above. Swapping
      // modes first left the two nodes with EXCHANGED modes but untouched positions
      // on the null-rect early-return — a FLOAT/TILE pair stranded the wrong window
      // in/out of tiling (renderTree treats node.mode as authoritative).
  ...
  _swappable(node) {
    if (!node) return false;
    // forge-r63u: swapPairs (the sole caller) resolves its target via a raw
    // this.next() sibling on the directional move/swap keybinding paths, which
    // runs synchronously pre-prune and can hand back a finalized wrapper; this
    // read is upstream of swapPairs's own isWindowAlive guards (1789/1793), so a
    // dead sibling's `.minimized` would throw here first. A dead window is not
  ...
  removeNode(node) {
    ...
    // forge-jdbe: test closedContainer (the container that actually lost a child),
    // NOT parentNode — in the single-child-collapse branch above parentNode is the
    // now-DETACHED sub-CON, so a nested split under a tabbed parent never un-tabbed
    // and writes landed on the detached node. Mirrors the forge-vw0l fix below; in
    // the non-collapse branch closedContainer === parentNode, so this is a no-op there.
    if (
      this.settings.get_boolean("auto-exit-tabbed") &&
      closedContainer.nodeType === NODE_TYPES.CON &&
      closedContainer.layout === LAYOUT_TYPES.TABBED &&
      closedContainer.childNodes.length === 1
    ) {
      ...
      closedContainer.layout = this.settings.get_boolean("auto-reorient-on-close")
        ? this.extWm.determineSplitLayoutForRect(closedContainer.rect)
        : this.extWm.determineSplitLayout();
      this.resetSiblingPercent(closedContainer);
      closedContainer.lastTabFocus = null;
    }
  ...
  /**
   * Concise, throw-safe one-line description of a node for violation reports.
   * WINDOW values can be finalized Meta.Windows, so every access is guarded.
   * @param {Node} node
   * @returns {string}
   */
  _describeNode(node) {
    ...
    let label = "";
    try {
      label = `${node.nodeType}#${node.index !== null ? node.index : "-"}`;
    } catch (e) {
      return "<unreadable-node>";
    }
    try {
      if (node.isWindow && node.isWindow()) {
        const w = node.nodeValue;
        const cls = w && w.get_wm_class ? w.get_wm_class() : "?";
        label += ` class:'${cls}'`;
      } else if (node.nodeValue != null) {
        label += ` value:'${node.nodeValue}'`;
      }
    } catch (e) {
  ...
  /**
   * Dev-build structural invariant checker (forge-fhen.5). Walks the tree from
   * ROOT over childNodes/parentNode and collects EVERY structural violation of
   * the canonical rule set so the reparent/splice/single-parent corruption class
   * (forge-bomy, forge-v2yz, forge-mo27) surfaces at runtime instead of via
   * manual audit. This is the in-memory-tree subset of the e2e oracle in
   * tests/e2e/framework/bridge.js `fuzzCheckInvariants()`; the geometry-overlap
   * and shape-metrics checks stay there.
   *
   * Rules (all collected — the walk never stops at the first hit):
   *  - parent-ref:      a child's parentNode !== the container it hangs under
   *  - duplicate-node:  the same node object is reachable via two distinct paths
   *  - cycle:           a node is its own ancestor
   *  - depth-exceeded:  tree depth > 256 (pathological/backstop)
   *  - empty-con:       a CON node with zero children
   *  - window-not-leaf: a WINDOW node with children
   *
   * IMPORTANT: this method body is intentionally UNGATED (no `production`
   * check) so bridge.js — which runs against prod-built e2e images — can call
   * it at runtime. Only the render() call site is dev-gated.
   *
   * @returns {{rule: string, node: string, ref: Node}[]} one entry per
   *   violation; the returned array is empty for a healthy tree.
   */
  verifyIntegrity() {
    const violations = [];
    const add = (rule, node) =>
      violations.push({ rule, node: this._describeNode(node), ref: node });

    // seen: every node reached anywhere (identity) -> duplicate detection.
    // ancestors: the nodes on the CURRENT root->node path -> cycle detection.
    // Splitting the two lets us tell a DAG-style duplicate apart from a true
    // self-ancestor cycle. depth>256 returns before recursing deeper so a
    // pathologically deep (but acyclic) tree can't blow the JS stack.
    const seen = new Set();

    const walk = (node, parent, depth, ancestors) => {
      if (!node) return;
      ...
      if (seen.has(node)) {
        add("duplicate-node", node);
        return;
      }
      seen.add(node);
      if (depth > 256) {
        add("depth-exceeded", node);
        return;
      }
      if (parent !== null && node.parentNode !== parent) {
        add("parent-ref", node);
      }
      const children = node.childNodes || [];
      if (node.isCon && node.isCon() && children.length === 0) {
        add("empty-con", node);
      }
      if (node.isWindow && node.isWindow() && children.length > 0) {
        add("window-not-leaf", node);
      }
      ancestors.add(node);
      for (let i = 0; i < children.length; i++) {
        walk(children[i], node, depth + 1, ancestors);
      }
      ancestors.delete(node);
    };

    walk(this, null, 0, new Set());
    return violations;
  }

  render(from) {
    // Dev-build structural invariant check (forge-fhen.5). Runs BEFORE cleanTree()
    // (invoked below via the render pipeline) because cleanTree removes empty CONs
    // and flattens single-CON chains, which would mask exactly the corruption this
    // is meant to catch. Log-and-continue: never throw, just surface it.
    if (!production) {
      const violations = this.verifyIntegrity();
      if (violations.length) {
        // The throw-safe _describeNode strings in each violation already identify
        // the offending nodes; do NOT call debugParentNodes here — its unguarded
        // Meta.Window access can throw on a finalized window and its parentNode
        // walk would hang on the very cycle verifyIntegrity is detecting, both of
        // which would break this feature's own "never throw" contract.
        Logger.error(
          `render: tree integrity FAILED (${violations.length}) from ${from || "?"}: ` +
            violations.map((v) => `${v.rule} @ ${v.node}`).join("; ")
        );
      }
    }
  ...
  _redistributeForMinSizes(sizes, mins, minTotal, totalSize) {
    ...
    if (minTotal >= totalSize) {
      for (let i = 0; i < sizes.length; i++) {
        // Guarantee at least 1px: a child with min 0 would otherwise floor to 0,
        // and apply() skips zero-size rects so the window is never placed and
        // keeps stale geometry overlapping its siblings (forge-l8av).
        sizes[i] = Math.max(1, Math.floor((mins[i] / minTotal) * totalSize));
      }
      // Trim any over-allocation (the +1 floors / rounding) off the largest
      // children down to totalSize, never below 1, so the sum stays exact and the
      // #330 remainder-fold in computeSizes() can't push a floored child back to 0.
      let over = sizes.reduce((a, b) => a + b, 0) - totalSize;
      while (over > 0) {
        let idx = -1;
        let best = 1;
        for (let i = 0; i < sizes.length; i++) {
        ...
        if (idx === -1) break; // every child already at the 1px floor
        sizes[idx]--;
        over--;
      }
      return;
