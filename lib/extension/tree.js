...
export class Node extends GObject.Object {
  ...
  _ensureConTab() {
    ...
    iconBin.child = rep.app.create_icon_texture(24 * Utils.dpi());
    // Keep title at child index 1 so Node.render() updates it like a window tab.
    let titleButton = new St.Button({ x_expand: true, label: `${this._getTitle()}` });
    ...
    let clickFn = () => {
      ...
      tabContents.add_style_class_name("window-tabbed-tab-active");
      this.getNodeByType(NODE_TYPES.WINDOW).forEach((w) => w.nodeValue?.raise());
      rep.nodeValue.activate(global.display.get_current_time());
    };
...
export class Tree extends Node {
  ...
  removeNode(node) {
    ...
    // MONITOR nodes are never removed (they persist with the workspace), but
    // their layout setting can be toggled by the user via LayoutToggle. When
    // the last child window leaves, reset the layout back to the
    // monitor-orientation default so the next batch of windows starts fresh
    // rather than inheriting a stale toggle from the previous occupants.
