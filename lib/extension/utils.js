...
/**
 * get_monitor_geometry() returns null for an invalid monitor index, and
 * get_current_monitor() can transiently return -1 during monitors-changed /
 * startup. Gate on the index and coerce a null return, so callers get one
 * null-check idiom instead of hand-rolling it at each site.
 */
export function getMonitorGeometrySafe(index) {
...
export function resolveX(rectRequest, metaWindow) {
  ...
  // forge-h0w3: every branch below is work-area relative and the tail adds
  // monitorRect.x back. The fallthrough defaults (an absent x, or an
  // unrecognised string) leave `val` at the ABSOLUTE frame x, which the tail
  // then double-offset — on a secondary monitor at x=1920 that returned
  // 1920 + win.x. Pre-subtracting the origin makes "unspecified" mean "leave the
  // window where it is", matching resolveWidth/resolveHeight.
  val = metaRect.x - monitorRect.x;
  let x = rectRequest.x;
  switch (typeof x) {
    case "string":
      switch (x) {
...
export function resolveY(rectRequest, metaWindow) {
  ...
  let val = metaRect.y;
  if (!monitorRect) return val;
  // forge-h0w3: see resolveX. On the primary monitor the work-area origin is the
  // top-panel height, so an omitted y pushed the window down on every call.
  val = metaRect.y - monitorRect.y;
  let y = rectRequest.y;
  switch (typeof y) {
    case "string":
      switch (y) {
...
export function findWindowWith(title) {
  ...
  // forge-m46q: Mutter workspace indices are 0..n-1. This used to run 1..n, so
  // workspace 0 was never scanned and the last iteration passed an out-of-range
  // index — masked only because get_workspace_by_index(n) returns null and
  // get_tab_list(type, null) means "all workspaces", so the accidental final
  // sweep covered workspace 0 at the cost of n redundant full scans.
...
export function _disableDecorations() {
  let decos = global.window_group
    .get_children()
    .filter((a) => /** @type {{ type?: string }} */ (a).type != null);
  decos.forEach((d) => {
...
export function dpi() {
  return St.ThemeContext.get_for_stage(
